...
provider "google" {
  ...
  credentials = file("<ENTER JSON CREDS PATH HERE>")

  project = "${var.gcp_project_id}"
  region  = "${var.gcp_region}"
  zone    = "${var.gcp_zone}"
}
resource "google_compute_instance" "master_test" {
...
provisioner "remote-exec" {
    ...
    connection {
        type        = "ssh"
        user        = "bot"
        host        = google_compute_instance.master_test.network_interface[0].access_config[0].nat_ip
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i '${google_compute_instance.master_test.network_interface[0].access_config[0].nat_ip},' <insert PATH TO site.yml file here> "
  }
}
